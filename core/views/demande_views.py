...
from django.contrib.auth.decorators import login_required  # ← depuis Django
...
from core.decorators import hopital_required, donneur_required  # ← sans login_required
...
def creer_demande(request):
    ...
    return render(request, 'demandes/form.html', {'form': form, 'titre': 'Nouvelle demande'})
...
def repondre_demande(request, pk):
    ...
    if not donneur.actif:
        messages.error(request, "Votre compte donneur est désactivé. Réactivez-le pour répondre.")
    ...
    if not donneur.est_eligible():
        messages.warning(
            request,
            f"Vous n'êtes pas encore éligible. Prochaine date : {donneur.get_prochain_don()}"
        )
        return redirect('detail_demande', pk=pk)
    ...
    if request.method == 'POST':
        ...
        messages.success(request, "Votre intention de don a été enregistrée. Merci !")
